...
public class Client_info_Controller implements Initializable {
    ...
    Button search;
    @FXML
    TableView<gui_classes.Client> table;
    @FXML
    TableColumn<gui_classes.Client, String> client_id, client_fname, client_lname, client_pass, client_email, client_phonenum, client_location, client_gender;
    @FXML
    TableColumn<Client, Image> client_image;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        client_id.setCellValueFactory(data -> {
            gui_classes.Client value = data.getValue();
        ...
        client_email.setCellValueFactory(data -> {
            Client value = data.getValue();
            return new ReadOnlyStringWrapper(((Client) value).getEmail());
        });
        client_pass.setCellValueFactory(data -> {
            Client value = data.getValue();
            return new ReadOnlyStringWrapper(((Client) value).getPassword());
        });

        client_phonenum.setCellValueFactory(data -> {
            Client value = data.getValue();
            return new ReadOnlyStringWrapper(((Client) value).getPhonenum());
        });
        client_gender.setCellValueFactory(data -> {
            Client value = data.getValue();
            return new ReadOnlyStringWrapper(((Client) value).getGender());
        });
        client_location.setCellValueFactory(data -> {
            Client value = data.getValue();
            return new ReadOnlyStringWrapper(((Client) value).getLocation());
        });
...
//                table.getItems().add(new Client(cid, fname, lname, email, pass, phonenum, gender, Clocation,image));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void updatefNameHandler(TableColumn.CellEditEvent event) {
        ...
        String edit = "call updateCFname('" + client.getId() + "','" + event.getNewValue() + "')";
        try {
            Class.forName("com.mysql.jdbc.Driver");
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/project_gui", "admin", "admin" + "");
            Statement stmt = con.createStatement();
    ...
    private void updatelNameHandler(TableColumn.CellEditEvent event) {
        Client client = table.getSelectionModel().getSelectedItem();
        String edit = "call updateCLname('" + client.getId() + "','" + event.getNewValue() + "')";
        try {
            Class.forName("com.mysql.jdbc.Driver");
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/project_gui", "admin", "admin" + "");
            Statement stmt = con.createStatement();
    ...
    private void updateemailHandler(TableColumn.CellEditEvent event) {
        Client client = table.getSelectionModel().getSelectedItem();
        String edit = "call update_email('" + client.getId() + "','" + event.getNewValue() + "')";
        try {
            Class.forName("com.mysql.jdbc.Driver");
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/project_gui", "admin", "admin" + "");
            Statement stmt = con.createStatement();
    ...
    private void updategenderHandler(TableColumn.CellEditEvent event) {
        Client client = table.getSelectionModel().getSelectedItem();
        String edit = "call update_gender('" + client.getId() + "','" + event.getNewValue() + "')";
        try {
            Class.forName("com.mysql.jdbc.Driver");
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/project_gui", "admin", "admin" + "");
            Statement stmt = con.createStatement();
    ...
    private void updatelocationHandler(TableColumn.CellEditEvent event) {
        Client client = table.getSelectionModel().getSelectedItem();
        String edit = "call update_location('" + client.getId() + "','" + event.getNewValue() + "')";
        try {
            Class.forName("com.mysql.jdbc.Driver");
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/project_gui", "admin", "admin" + "");
            Statement stmt = con.createStatement();
    ...
    private void updatephonenumHandler(TableColumn.CellEditEvent event) {
        Client client = table.getSelectionModel().getSelectedItem();
        String edit = "call update_phonenum('" + client.getId() + "','" + event.getNewValue() + "')";
        try {
            Class.forName("com.mysql.jdbc.Driver");
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/project_gui", "admin", "admin" + "");
            Statement stmt = con.createStatement();
...
//                    while (rs.next()) {
//                        table.getItems().add(new Client (rs.getInt(2), rs.getString(3), rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getString(9),rs.getByte(10)));
//                    }
